...
if len(data) < 5:
    raise SystemExit("No hay suficientes filas limpias para entrenar target_1d.")
...
next_day_pred = float(model.predict(last_features)[0])
last_price = float(data.iloc[-1]["official_price"])

if next_day_pred > last_price + 0.25:
    direction = "SUBE"
elif next_day_pred < last_price - 0.25:
    direction = "BAJA"
else:
    direction = "LATERAL"
...
print("predicted_target_1d:", round(next_day_pred, 4))
print("direction_1d:", direction)
